...
} from '../../../src/Application/UseCases/Auth/LoginUseCases'
...
describe('AuthPloc', () => {
  ...
  describe('init()', () => {
    it('should set UNAUTHENTICATED when no session exists', async () => {
      checkAuthSessionUseCase.execute.mockResolvedValue({
    ...
    it('should refresh token if session needs refresh', async () => {
      ...
      refreshTokenUseCases.execute.mockResolvedValue({
      ...
      expect(refreshTokenUseCases.execute).toHaveBeenCalledWith({
        refreshToken: 'refresh-token',
      })
      expect(ploc.state.status).toBe(AuthStatus.AUTHENTICATED)
    })
  ...
  describe('register()', () => {
    it('should set UNAUTHENTICATED after successful registration', async () => {
      registerUseCases.execute.mockResolvedValue({
