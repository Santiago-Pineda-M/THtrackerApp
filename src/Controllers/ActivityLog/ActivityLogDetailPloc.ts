...
import type { GetActivityLogByIdUseCase, UpdateActivityLogUseCase, SaveActivityLogValuesUseCase, GetActivityLogValuesUseCase } from '../../Application/UseCases/ActivityLog';
...
export class ActivityLogDetailPloc extends Ploc<IActivityLogDetailState> {
    ...
    async getLogDetail(id: string) {
        this.changeState({ ...this.state, isLoading: true, error: null, success: false, message: '' });
    ...
    async saveValues(id: string, values: LogValueRequest[]) {
        ...
        const result = await this.saveActivityLogValuesUseCase.execute({ id, requests: values });
