...
export const AuthSessionExample: React.FC = () => {
    ...
    const getStateRows = (): Record<string, React.ReactNode>[] => {
        ...
        if (authState.user) {
            ...
            rows.push(
                ...
                { key: 'user.accessToken', value: session.accessToken ? `${session.accessToken.substring(0, 30)}...` : 'N/A' },
                { key: 'user.refreshToken', value: session.refreshToken ? `${session.refreshToken.substring(0, 30)}...` : 'N/A' },
                { key: 'user.accessTokenExpiresAt', value: session.accessTokenExpiresAt ? new Date(session.accessTokenExpiresAt).toLocaleString() : 'N/A' },
                { key: 'user.refreshTokenExpiresAt', value: session.refreshTokenExpiresAt ? new Date(session.refreshTokenExpiresAt).toLocaleString() : 'N/A' },
                { key: 'user.isValid', value: session.isValid() ? 'true' : 'false' },
                { key: 'user.isAccessTokenExpired', value: session.isAccessTokenExpired() ? 'true' : 'false' },
                { key: 'user.isRefreshTokenExpired', value: session.isRefreshTokenExpired() ? 'true' : 'false' }
            );
        } else {
    ...
    return (
        <Card title="Auth Session State" w={2} h={3}>
