...
export const ActivityLogs: React.FC<ActivityLogsProps> = ({ activityId }) => {
    ...
    return (
        <Card title="Historial de Registros" h={3} w={3}>
                        ...
                        {state.logs.map((log) => (
                            ...
                            >
                                <div className={styles.timeInfo}>
                                    <Text weight="medium">{new Date(log.startedAt).toLocaleDateString()} {new Date(log.startedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
                                    {log.endedAt && (
                                        <Text size="xs" color="secondary">
                                            Fin: {new Date(log.endedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                        </Text>
                                    )}
                                </div>
                                <div className={styles.statusContainer}>
                                    <Badge variant={log.durationMinutes ? 'default' : 'success'}>
                                        {log.durationMinutes ? `${log.durationMinutes} min` : 'En curso'}
                                    </Badge>
                                </div>
                            </div>
                        ))}
