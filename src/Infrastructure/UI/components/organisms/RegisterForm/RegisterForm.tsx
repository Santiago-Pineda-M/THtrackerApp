...
export const RegisterForm: React.FC = () => {
    ...
    return (
        <Card w={2} h={3} title="Crear Cuenta">
            <div className={s.form}>
                {state.message && (
                    <Text size="sm" className={state.success ? s.success : s.error}>
                        {state.message}
                    </Text>
                )}

                <FormField label="Nombre Completo" required error={state.errors.name?.[0]}>
                    <Input
                        name="name"
                        type="text"
                        value={state.name}
                ...
                <FormField label="Correo Electrónico" required error={state.errors.email?.[0]}>
                    <Input
                        name="email"
                        type="email"
                        value={state.email}
                ...
                <FormField
                    label="Contraseña"
                    required
                    error={!state.errors.password?.length ? undefined : state.errors.password?.[0]}
                >
                    <Input
                        name="password"
                        type="password"
                ...
                <FormField label="Confirmar Contraseña" required error={state.errors.confirmPassword?.[0]}>
                    <Input
                        name="confirmPassword"
                        type="password"
                        value={state.confirmPassword}
                        onChange={(e) => providerRegisterPloc.updateConfirmPassword(e.target.value)}
                        placeholder="••••••••"
                        disabled={state.isLoading}
                    />
                </FormField>
                ...
                <Text size="xs" className={s.register}>
