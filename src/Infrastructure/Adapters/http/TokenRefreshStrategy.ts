...
export type OnSessionRefreshedFn = (newAccessToken: string, response: IRefreshTokenResponse) => Promise<void>;
...
export class TokenRefreshStrategy {
     ...
     */
    async attemptRefresh<T>(retryRequest: (newAccessToken: string) => Promise<T>): Promise<T> {
        if (this.isRefreshing) {
            ...
            return new Promise<T>((resolve, reject) => {
                this.subscribers.push({
                    ...
                    reject: (err) => {
                        console.log('[AuthRefresh] ❌ Petición encolada fallida por error en refresh');
                        reject(err);
                    },
                });
            });
        ...
        try {
            ...
            if (response.status !== 200) {
                ...
                throw new Error(`Refresh fallido (${response.status}): ${errorData.detail || 'Error desconocido'}`);
            }

            const result: IRefreshTokenResponse = await response.json();
            if (!result.accessToken || !result.refreshToken) throw new Error('Respuesta de refresh incompleta');

            // Delegar la persistencia al repositorio a través del callback
            await this.onSessionRefreshed(result.accessToken, result);
            console.log('[AuthRefresh] 💾 Nuevos tokens persistidos en el repositorio');

            // Notificar éxito a todas las peticiones encoladas
            console.log(`[AuthRefresh] 📢 Notificando a ${this.subscribers.length} suscriptores...`);
            this.notifySubscribers(result.accessToken);

            return retryRequest(result.accessToken);
        } catch (error) {
            console.error('[AuthRefresh] 🚨 Error fatal en el proceso de refresco:', error);
