...
export class UserService implements IUserService {
    ...
    async getProfile(): Promise<UserProfileResponse | ProblemDetails> {
        try {
            const response = await this.httpClient.get<UserProfileResponse | ProblemDetails>(`${this.baseUrl}/me`);
    ...
    async updateProfile(request: UpdateUserProfileRequest): Promise<UserProfileResponse | ProblemDetails> {
        try {
            const response = await this.httpClient.put<UserProfileResponse | ProblemDetails>(`${this.baseUrl}/me`, request);
            if (response.status === 200) return response.data as UserProfileResponse;
            return response.data as ProblemDetails;
        } catch (error) {
            return this.toNetworkError(error);
