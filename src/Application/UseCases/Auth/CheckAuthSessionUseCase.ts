...
import type { IUseCase, IRefreshTokenRequest, IRefreshTokenResponse } from '../../../Domain';
...
export class CheckAuthSessionUseCase implements IUseCase<void, CheckAuthSessionOutput> {
    ...
    async execute(): Promise<CheckAuthSessionOutput> {
        ...
        if (session.isAccessTokenExpired() && !session.isRefreshTokenExpired()) {
            console.log('[CheckAuthSession] 🔄 AccessToken expirado. Intentando renovar con refreshToken...');
     ...
     */
    private async attemptTokenRefresh(currentSession: AuthSession): Promise<AuthSession | null> {
        try {
    ...
    private isRefreshSuccess(r: IRefreshTokenResponse | unknown): r is IRefreshTokenResponse {
        return r !== null && typeof r === 'object' && 'accessToken' in r && 'refreshToken' in r;
    }
}
